...
use crate::builtins::matrix::Matrix;
use crate::builtins::path::Path;
use crate::traits::Share;

#[derive(Clone, Debug, Serialize, Deserialize)]
#[repr(C)]
pub struct Image(pub Cell<Inner>);

#[derive(Clone, Debug)]
pub struct Inner(pub DynamicImage);
...
impl Image {
    ...
    pub fn resize(self, new_w: u32, new_h: u32) -> Self {
        Image(Cell::new(Inner(self.0.resize_exact(
            new_w,
            new_h,
    ...
    pub fn into_matrix_rgba(self) -> Matrix<f32> {
        let w = self.0.width() as usize;
        let h = self.0.height() as usize;
        let mut array = ArrayBase::zeros(vec![4, w, h]);
        for (x, y, rgb) in self.0.pixels() {
            let x = x as usize;
            let y = y as usize;
            array[[0, x, y]] = rgb[0] as f32;
    ...
    pub fn draw_box(mut self, x: u32, y: u32, w: u32, h: u32, rgba: Array<u8, 4>) {
        ...
        for i in 0..w {
            self.0.put_pixel(x + i, y, rgba);
            self.0.put_pixel(x + i, y + h, rgba);
        }
        for i in 0..h {
            self.0.put_pixel(x, y + i, rgba);
            self.0.put_pixel(x + w, y + i, rgba);
        }
    }
}
