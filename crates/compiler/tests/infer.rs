...
fn test_infer_def_recursive0() {
    ...
    let b = infer(aqua!("def f[T](x: T): T = (f[T]:T => T)(x:T):T;")).unwrap();
    check!(a, b);
}
...
fn test_infer_impl_assoc2() {
    ...
    let b = infer(aqua!(
        "trait Foo[T] { def f[U](x:T, y:U): T; }
         ...
         ((Foo[i32]::f[i32]):((i32,i32) => i32))(0:i32, 1:i32):i32;"
    ))
    .unwrap();
    check!(a, b);
}
...
fn test_infer_impl_where_bound4() {
    ...
    let b = infer(aqua!(
        "trait Foo[T] { def f(x: T): T; }
         def g[T](x: T): T where Foo[T] = (g[T]:T => T)(x:T):T;"
    ))
    .unwrap();
    check!(a, b);
}
...
fn test_infer_trait_def0() {
    ...
    let b = infer(aqua!(
        "trait Foo[T] { def f(x: T): T; }
         impl Foo[i32] { def f(x: i32): i32 = x:i32; }
         def g[T](x: T): T where Foo[T] = (Foo[T]::f:T => T)(x:T):T;"
    ))
    .unwrap();
    check!(a, b);
}
...
fn test_infer_i32_add3() {
    ...
    let b = infer(aqua!(
        "Add[i32,i32]::add(
             Add[i32,i32]::add(1:i32, 2:i32):i32,
             3:i32
         ):i32;"
    ))
    .unwrap();
...
#[test]
fn test_infer_i32_display0() {
    let a = infer(aqua!("Display[i32]::toString(1:i32);")).unwrap();
    let b = infer(aqua!("Display[i32]::toString(1:i32):String;")).unwrap();
    check!(a, b);
}

#[test]
fn test_infer_i32_display1() {
    let a = infer(aqua!("Display[_]::toString(1:_);")).unwrap();
    let b = infer(aqua!("Display[i32]::toString(1:i32):String;")).unwrap();
    check!(a, b);
}

#[test]
fn test_infer_i32_display2() {
    let a = infer(aqua!("1.toString();")).unwrap();
    let b = infer(aqua!("Display[i32]::toString(1:i32):String;")).unwrap();
    check!(a, b);
}

#[test]
fn test_infer_vec_i32_display0() {
...
fn test_infer_vec_i32_display2() {
    let a = infer(aqua!("Display[Vec[_]]::toString(Vec[i32]::new());")).unwrap();
    let b = infer(aqua!(
        "Display[Vec[i32]]::toString(Vec[i32]::new()):String;"
    ))
    .unwrap();
...
#[ignore]
fn test_infer_for() {
    let a = infer(aqua!("for i in 0..10 { i; }")).unwrap();
    let b = infer(aqua!("for i in IntoIterator[Item=i32,IntoIter=VecIterator[i32]]::into_iter(Range[i32]::range(0:i32, 10:i32) { i:i32; }")).unwrap();
    check!(a, b);
}
