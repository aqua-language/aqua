...
//     ctx.trait_impls = [impls.to_vec()];
//
//     assert!(ctx.solve(&goal, &[]).is_some());
// }
//
// // impl Clone[i32] {}
// // Goal: Clone[i32]
...
// fn test_trait2() {
//     let mut ctx = Context::new();
//     ctx.trait_impls = impls([impl_clone([], ty_i32(), [])]);
//     let goal = tr_clone(ty_i32());
//
//     assert!(ctx.solve(&goal, &[]).is_some());
// }
...
//     let mut ctx = Context::new();
//     ctx.trait_impls = impls([impl_iterator(["T"], ty_vec(ty_gen("T")), ty_gen("T"), [])]);
//     assert!(ctx.solve(&goal, &[]).is_some());
